...
function createTemplativeApp() {
    ...
    pipenv run pyinstaller --distpath ../bin -y -n templative --log-level=DEBUG --onefile ./templative/__main__.py --hidden-import=engineio.async_drivers.aiohttp --hidden-import=engineio.async_aiohttp --collect-all templative --add-data "./templative/lib/create/componentTemplates;templative/lib/create/componentTemplates" --codesign-identity "Developer ID Application: Next Day Games LLC (Y9RWBVMY7R)"
    cd ..
}

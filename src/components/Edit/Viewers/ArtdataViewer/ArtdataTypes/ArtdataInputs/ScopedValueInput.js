...
export default class ScopedValueInput extends React.Component {   
    createSelectionOption = (scope) => {
        const defaultInput = <input type="text" className="form-control" onChange={(event)=>this.props.updateArtdataFieldCallback(this.props.index, "source", event.target.value)} aria-label="What key to get from the scope..." value={this.props.source}/>

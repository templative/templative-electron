...
const USER_EMAIL = "arijaerg@gmail.com";

async function wireUpComponent(description, files) {
    const fileInclusions = files.map(file => `${file.type} at ${file.filepath}\n'''\n${file.contents}\n'''`).join("\n\n");
    const desiredResponseFormat = '[{"filepath": "path/to/file", "newContents": "updatedContents"}]';
...
Each of the included files should have a file in the response and the updates you made to it.`;
    ...
    try {
        const [response_data, status] = await promptChatGPT(
            system_prompt,
        );
        
        if (status !== 200) {
            console.log(`Error from API: ${response_data.message || 'Unknown error'}`);
            return [];
        }
            
        return response_data.message;
    } catch (e) {
        console.log(`Error enhancing component: ${e.toString()}`);
        return [];
    }
}

async function enhanceArtdata(artdata, description) {
    const system_prompt = `
    ...
    try {
        const [response_data, status] = await updateTemplativeFileForDescription(
            artdata,
            system_prompt,
            description
        );
        ...
        const enhanced_artdata = JSON.parse(response_data.message);
        return enhanced_artdata;
    } catch (e) {
        console.log(`Error enhancing artdata: ${e.toString()}`);
        return artdata;
    }
}

async function enhanceSvg(svg_content, description, artdataFileContents) {
    let system_prompt = `
    ...
    `;
    system_prompt += `Artdata: ${artdataFileContents}`;
    ...
    try {
        ...
        console.log(`Error enhancing SVG: ${e.toString()}`);
        return svg_content;
    }
}

async function enhanceComponentJson(componentJson, description) {
    const system_prompt = `
    ...
    try {
        ...
        return JSON.parse(response_data.message);
    } catch (e) {
        console.log(`Error enhancing component JSON: ${e.toString()}`);
        return componentJson;
    }
}

async function enhancePiecesJson(piecesJson, description) {
    const system_prompt = `
    ...
    try {
        ...
        console.log(`Error enhancing pieces JSON: ${e.toString()}`);
        return piecesJson;
    }
}

module.exports = { wireUpComponent, enhanceArtdata, enhanceSvg, enhanceComponentJson, enhancePiecesJson };